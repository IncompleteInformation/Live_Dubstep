...
int accel_to_serial(float accel){
  accel+=1;accel/=2;accel*=127;
  if (accel > 127.){accel = 127.;}
  if (accel < 0.  ){accel = 0.  ;}
  return 127 - (int) accel;
}
...
//  delay(10);
  /*for (int i = 0 ; i < 3 ; i++)
  {
    mySerial.write((i+4)+128);mySerial.write(accel_to_serial(accelG[i]));
    //Serial.print(accel_to_serial(accelG[i]));
